...
function NumInput({label, value, onChange, min=0, step=1}:{label:string; value:number; onChange:(v:number)=>void; min?:number; step?:number}){
  return (
    <label className="block mb-3">
      ...
      <input className="glass-input mt-1" type="number" value={isFinite(value) ? value : 0} min={min} step={step}
...
function PctInput({label, value, onChange}:{label:string; value:number; onChange:(v:number)=>void}){
  return (
    <label className="block mb-3">
      ...
      <input className="w-full" type="range" min={0} max={100} value={value} onChange={(e)=> onChange(parseFloat(e.target.value))}/>
    </label>
  );
}
  ...
  return (
    <div className="card p-4 md:p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <NumInput label="Attendees" value={inputs.attendees} onChange={(v)=> setInputs({ attendees: Math.max(0, v) })} />
        <PctInput label="% Drinkers" value={inputs.percentDrinkers} onChange={(v)=> setInputs({ percentDrinkers: Math.max(0, Math.min(100, v)) })} />
        <PctInput label="% Eating" value={inputs.percentEating} onChange={(v)=> setInputs({ percentEating: Math.max(0, Math.min(100, v)) })} />
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="btn" onClick={()=> setInputs({ otherCosts: [...inputs.otherCosts, { label: 'Cost', amount: 0 }] })}>Add</button>
        </div>
        <div className="space-y-2">
          {inputs.otherCosts.map((c, idx)=> (
            <div key={idx} className="grid grid-cols-9 gap-2">
              <input className="glass-input col-span-5" value={c.label} onChange={(e)=> {
                const arr = inputs.otherCosts.slice(); arr[idx] = { ...c, label: e.target.value }; setInputs({ otherCosts: arr });
              }}/>
              <input className="glass-input col-span-3" type="number" step={0.01} value={c.amount}
                onChange={(e)=> {
                  const arr = inputs.otherCosts.slice(); arr[idx] = { ...c, amount: parseFloat(e.target.value) }; setInputs({ otherCosts: arr });
                }}/>
              <button className="btn col-span-1" onClick={()=> {
                const arr = inputs.otherCosts.slice(); arr.splice(idx,1); setInputs({ otherCosts: arr });
              }}>✕</button>
            </div>
          ))}
        </div>
      ...
      <button className="mt-4 text-sm text-brand-accent underline" onClick={()=> setShowAdv(v=>!v)}>
