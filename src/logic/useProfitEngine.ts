...
export function calculate(inputs: Inputs): Results {
  ...
  const maxOccupancy = Math.max(0, nz(inputs.maxOccupancy, 'maxOccupancy'));
  const attendancePercent = Math.max(0, Math.min(100, nz(inputs.attendancePercent, 'attendancePercent')));
  const attendees = Math.round(maxOccupancy * (attendancePercent / 100));
  ...
  // Ticket revenue net of per-ticket fee
